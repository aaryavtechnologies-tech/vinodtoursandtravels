...
(async()=>{
 ...
 console.log(JSON.stringify({pages:56,widths:[320,390,768,1024,1440],issues,interactions:'passed'},null,2));fs.writeFileSync('reference/ui-results.json',JSON.stringify(issues,null,2));process.exitCode = issues.length ? 1 : 0; await b.close();
